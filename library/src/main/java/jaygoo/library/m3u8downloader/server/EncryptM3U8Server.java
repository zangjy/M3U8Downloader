...
public class EncryptM3U8Server extends M3U8HttpServer {
    public void encrypt() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    M3U8EncryptHelper.encryptTsFilesName(M3U8Downloader.getInstance().getEncryptKey(), filesDir);
                } catch (Exception e) {
    ...
    public void decrypt() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    M3U8EncryptHelper.decryptTsFilesName(M3U8Downloader.getInstance().getEncryptKey(), filesDir);
                } catch (Exception e) {
